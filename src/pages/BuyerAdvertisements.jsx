...
import { buyerAdvertisementService } from '../services/buyerAdvertisementService';
import { API_CONFIG } from '../config/config';

const BuyerAdvertisements = () => {
  ...
  const fetchAdvertisements = async () => {
    try {
      setError(null);
      setLoading(true);
      const response = await buyerAdvertisementService.getAll();
      let ads = [];
      if (response?.data) {
        if (Array.isArray(response.data)) ads = response.data;
        else if (response.data.ads && Array.isArray(response.data.ads)) ads = response.data.ads;
        else if (response.data.data && Array.isArray(response.data.data)) ads = response.data.data;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingAd) {
        await buyerAdvertisementService.update(editingAd._id, formDataToSend);
      } else {
        await buyerAdvertisementService.create(formDataToSend);
      }
      setShowModal(false);
      setEditingAd(null);
      setFormData({
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this advertisement?')) {
      try {
        await buyerAdvertisementService.delete(id);
        fetchAdvertisements();
      } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Buyer Advertisements</h1>
        <div className="flex gap-2">
          <a href="/advertisements/seller" className="px-3 py-2 border rounded">Seller</a>
          <a href="/advertisements/buyer" className="px-3 py-2 border rounded bg-gray-200">Buyer</a>
      ...
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
          ...
          <p>{error}</p>
          <button onClick={fetchAdvertisements} className="mt-2 text-red-700 underline hover:text-red-900">
            Retry
          </button>
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4">{editingAd ? 'Edit Advertisement' : 'Add Advertisement'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2">Title</label>
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required className="w-full border rounded p-2" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Description</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} required className="w-full border rounded p-2" rows="3" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Media Type</label>
                <select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} className="w-full border rounded p-2" required>
              ...
              <div>
                <label className="block text-sm font-medium mb-2">{editingAd ? 'New Media (Video/Image)' : 'Media (Video/Image)'}</label>
                <input type="file" accept={formData.type === 'video' ? 'video/*' : 'image/*'} onChange={(e) => setFormData({ ...formData, media: e.target.files[0] })} className="w-full border rounded p-2" />
                {editingAd && !formData.media && <p className="text-xs text-gray-500 mt-1">Leave empty to keep current media</p>}
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">{editingAd ? 'New Thumbnail' : 'Thumbnail'}</label>
                <input type="file" accept="image/*" onChange={(e) => setFormData({ ...formData, thumbnail: e.target.files[0] })} className="w-full border rounded p-2" />
                {editingAd && !formData.thumbnail && <p className="text-xs text-gray-500 mt-1">Leave empty to keep current thumbnail</p>}
              </div>
              <div className="flex justify-end space-x-4">
                <button type="button" onClick={() => { setShowModal(false); setEditingAd(null); setFormData({ title: '', description: '', type: 'image', isFeatured: false, media: null, thumbnail: null, }); }} className="px-4 py-2 border rounded hover:bg-gray-100">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">{editingAd ? 'Update' : 'Create'}</button>
              </div>
            </form>
          </div>
        </div>
